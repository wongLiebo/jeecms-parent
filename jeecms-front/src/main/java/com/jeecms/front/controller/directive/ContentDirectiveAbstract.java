...
public abstract class ContentDirectiveAbstract implements TemplateDirectiveModel {
    ...
    /**
     * 输入参数，栏目选项。用于单栏目情况下。0：自身栏目；1：包含子栏目；2：包含引用内容。 效率 0【查询栏目数少的情况下】 最高 1其次  2最低
     */
    ...
    Integer getChannelOption(Map<String, TemplateModel> params) throws TemplateException {
        Integer channelOption = DirectiveUtils.getInt(CHANNEL_OPTION, params);
    ...
    List<Integer> getContentSecretIds(CoreUser user) {
        List<Integer> contentSecretIds = new ArrayList<>();
        if (user != null) {
    ...
    List<Integer> getAnnexSecretIds(CoreUser user) {
        List<Integer> annexSecretIds = new ArrayList<>();
        if (user != null) {
            SysUserSecret userSecret = user.getUserSecret();
            if (userSecret != null) {
                annexSecretIds = new ArrayList<>(userSecret.getAnnexSecretIds());
            }
        }
        return annexSecretIds;
    }

    Content initAnnexSecret(List<Integer> annexSecretIds, Content bean) {
