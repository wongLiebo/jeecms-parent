		...
		try {
			if (ids.size() == 1) {
				...
				if(spaceData!=null){
					...
					File file =service.getFileWithFilename(spaceData, null);
					if(file.exists()){
				...
				String filename = "";
				List<Zipper.FileEntry> list = new ArrayList<>();
				...
				for (Integer id : ids) {
					...
					if(spaceData!=null){
						...
						list.addAll(export(spaceData,resAliasMap.get(spaceData.getId())));
					}
				}
