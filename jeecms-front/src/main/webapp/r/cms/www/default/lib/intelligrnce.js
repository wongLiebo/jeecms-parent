...
// 初始化
function layUiUse (id,url,trUrl,isId){
    formId = id
    srcUrl = url||''
            ...
            success: function(res){
                if(res.code == 200){
                    ...
                    for(var i = 0; i<listData.length; i++){
                        // 组件配置
                        var content = JSON.parse(listData[i].content);
                        var defValue = listData[i].defValue?JSON.parse(listData[i].defValue):''
                        // 列表开始创建
                        ...
                        if(layoutTypes.indexOf(fieldType) > -1){
                            formItemHtml = '<div class="layui-form-item standard-type"><div id="'+listData[i].field+'" style="position:relative"></div></div>'
                            getItemData(listData[i],fieldType,componentConfig);
                        } else {
                            /*表单组件*/
                ...
                success:function(res){
                    if(res.code == 200){
                        ...
                        if(res.data.processType == 2){
					        ...
					        $('.mode-message').html(res.data.prompt||'提交成功，感谢您的参与！');
                        }
