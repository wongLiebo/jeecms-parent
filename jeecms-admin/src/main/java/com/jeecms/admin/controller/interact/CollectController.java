...
public class CollectController extends BaseTreeAdminController<Channel, Integer> {
    ...
    private void doSaveContent(String identity,String appId,JSONArray contentJson,CmsSite site,
        ...
        List<CmsModelItem> items = model.getItems().stream().filter(m->CmsModelConstant.CONTENT_TXT.equals(m.getDataType())).sorted(Comparator.comparing(CmsModelItem::getId)).collect(Collectors.toList());
        items =items.stream().sorted(Comparator.comparing(CmsModelItem::getId)).collect(Collectors.toList());
        if (items != null && items.size() > 0) {
            defTxtModelField = items.get(0).getField();
        }
        String defPicModelField = null;
        List<CmsModelItem> picFields = model.getItems().stream().filter(m->CmsModelConstant.MANY_CHART_UPLOAD.equals(m.getDataType())).sorted(Comparator.comparing(CmsModelItem::getId)).collect(Collectors.toList());
        picFields =picFields.stream().sorted(Comparator.comparing(CmsModelItem::getId)).collect(Collectors.toList());
        ...
        if(contentJson!=null&&contentJson.size()>0){
            for (int i=0;i<contentJson.size();i++){
                ...
                if(StringUtils.isNotBlank(rs.getTitle())){
                    ...
                    String txt2Prefix = "content";
                    for(String key:otherKey){
                        String data = dataMap.get(key);
                        if(key.startsWith(txtPrefix)||key.startsWith(txt2Prefix)){
                            /**正文有图片则下载替换*/
