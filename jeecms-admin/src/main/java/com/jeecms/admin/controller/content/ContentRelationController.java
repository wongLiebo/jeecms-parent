...
public class ContentRelationController extends BaseController<ContentRelation, Integer> {
    ...
    public ResponseInfo sort(@RequestBody @Validated(value = ContentRelationSort.class) ContentRelationDto dto,
        ...
        // 如果源目标和目的目标相同(将A相关内容排序到A相关内容的前或者后)这种情况过滤掉不处理
        if (dto.getContentRelationId().equals(dto.getContentRelationIds().get(0))) {
    ...
    public ResponseInfo delete(@RequestBody @Valid DeleteDto dto, BindingResult result) throws GlobalException {
        ...
        Integer contentIdNum = contentRelations.stream().collect(Collectors.groupingBy(ContentRelation::getContentId)).size();
        if (contentIdNum > 1) {
            return new ResponseInfo(false);
        }
        if (!contentService.validType(CmsDataPerm.OPE_CONTENT_EDIT, contentService.findById(contentRelations.get(0).getContentId()).getChannelId())) {
