...
public class ContentLuceneServiceImpl implements ContentLuceneService, ContentListener {
	...
	@Override
	public synchronized boolean createIndex(Integer id) throws GlobalException {
	...
	@Override
	public synchronized boolean createIndexBatch(Iterable<Content> contents) throws GlobalException {
		boolean succ = true;
		IndexWriter writer = null;
		DirectoryTaxonomyWriter taxoWriter = null;
		try {
	...
	@Override
	public synchronized boolean updateIndexById(Integer id) throws GlobalException {
		Content content = contentService.findById(id);
		return updateIndexByContent(content);
	}

	@Override
	public synchronized  boolean updateIndexByContent(Content content) throws GlobalException {
		IndexWriter writer = null;
		DirectoryTaxonomyWriter taxoWriter = null;
		try {
			/**
			...
			writer = getIndexWriter();
			// 采用更新更好（有则修改，没有则创建），数据量小的情况下和先判断索引是否存在再添加差不多
			System.out.println(MyDateUtils.formatDate(content.getReleaseTime(),MyDateUtils.COM_Y_M_D_H_M_S_PATTERN));
			Document doc = ContentLuceneApache.createDocument(content);
			/** 写入索引的同时写入taxo索引 */
			taxoWriter = getTaxoIndexWriter();
			doc = facetsConfig.build(taxoWriter, doc);
			...
			logger.error(e.getMessage());
			// 保存未更新lucene记录
			luceneErrorService.saveError(content.getId(), ContentLuceneError.OP_UPDATE);
		} finally {
			if (taxoWriter != null) {
				try {
					taxoWriter.commit();
	 ...
	 */
	@Override
	public synchronized boolean resetIndex(Integer channelId, Integer siteId, Date releaseTimeStart, Date releaseTimeEnd)
			throws GlobalException {
		IndexWriter writer = null;
		/** 重置之前删除全部索引 */
		try {
	...
	@Override
	public synchronized boolean delete(Integer id) throws GlobalException {
		IndexWriter writer = null;
		boolean succ = true;
		try {
			writer = getIndexWriter();
	...
	@Override
	public synchronized boolean delete(Iterable<Integer> ids) throws GlobalException {
		IndexWriter writer = null;
		boolean succ = true;
		try {
			openDir();
	...
	@Override
	public synchronized boolean deleteIndexBatch(List<Integer> channelIds, Integer siteId) throws GlobalException {
		IndexWriter writer = null;
		boolean succ = true;
		try {
			openDir();
	...
	public Page<Content> searchPage(String keyword, SearchPosition searchPos, SearchKeyCondition keyCondition,
		...
		try {
			...
			try {
				...
				total = contentIdPage.getTotalElements();
			}catch (Exception e){
				logger.error(e.getMessage());
			}
			contents.addAll(contentList);
			contentPage = new PageImpl<Content>(contents, pageable, total);
		} catch (IOException e) {
			logger.error(e.getMessage());
		} finally {
			try {
	...
	private synchronized DirectoryTaxonomyWriter getTaxoIndexWriter() throws IOException {
		openTaxoDir();
