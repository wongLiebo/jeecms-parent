...
public class ContentLuceneApache {
	...
	public static Document createDocument(Content content) {
		...
		Integer recycleVal = ZERO;
		if (content.getRecycle()) {
			recycleVal = ONE;
		}
		doc.add(new StringField(RECYCLE, recycleVal.toString(), Field.Store.YES));
	...
	public static Builder createQueryBuild(String keyword, Boolean keyIsTermQuery, SearchPosition searchPos,
		...
		Builder queryBuild = new BooleanQuery.Builder();
		/**默认回收站内容不搜索*/
		Query recycleQ = new TermQuery(new Term(RECYCLE, ZERO.toString()));
		BooleanClause recycleBc = new BooleanClause(recycleQ, Occur.MUST);
		queryBuild.add(recycleBc);

		if (!StringUtils.isBlank(keyword)) {
			if (keyIsTermQuery != null && keyIsTermQuery) {
				/** 目前精准查询只用于标题 */
				if (SearchPosition.title.equals(searchPos)) {
					Query query = new TermQuery(new Term(TITLE_STR, keyword));
