...
public class UploadService {
	...
	public boolean isRunFfmpeg() {
		return  MediaUtil.isRun(ffmpegPath);
	}
	...
	public UploadResult doUpload(MultipartFile file, Boolean mark, String uploadPath, ResourceType resourceType,
		...
		if(StringUtils.isNotBlank(origName)){
			/** 去除格式后缀 */
			int lastIndex = origName.lastIndexOf(".");
			if(lastIndex!=-1){
				origName = origName.substring(0,lastIndex);
			}
		}
		if(StringUtils.isBlank(origName)){
			origName = String.valueOf(System.currentTimeMillis() + 1000);
		}
		//直接刷新列表上传的文件名是 undefined
		...
		try {
			...
			/** 如果是音频或视频, 提取时长 */
			duration = getDuration(new FileInputStream(tempFile), resourceType, fileName);
			...
			/**视频抽帧*/
			if(((ResourceType.MEDIA.equals(resourceType) || MediaUtil.isVideo(ext))) && isRunFfmpeg()){
				String tempPath = FileUtils.getTempPath();
				File tempFileForCutCover = new File(tempPath, String.valueOf(System.currentTimeMillis() + 2000)+".jpg");
				...
				videoCover = doUpload(tempFileForCutCover,null,null,ResourceType.IMAGE,site);
				ResourcesSpaceData videoCoverRes = resourcesService.save(userId, spaceId, videoCover.getOrigName(),
						videoCover.getFileSize().intValue(), videoCover.getFileUrl(), videoCover.getDimensions(),
						videoCover.getResourceType(), false,site.getId(), duration, ftp, oss, null);
		...
		if (StringUtils.isNoneBlank(fileUrl)) {
			...
			uploadInfo.setVideoCover(videoCover);
		} else {
			log.error("未知上传错误 ");
			throw new GlobalException(new UploadExceptionInfo());
		}
	...
	private void validate(MultipartFile file, String uploadPath, ResourceType resourceType, Integer modelId,
		...
		if(MediaUtil.isVideo(filename)){
			resourceType = ResourceType.MEDIA;
		}
		if(MediaUtil.isMusic(filename)){
		...
		if (modelId != null && StringUtils.isNoneBlank(fieldName)) {
			...
			if(resourceType!=null){
				if (ResourceType.IMAGE.equals(resourceType)) {
					...
					/** 验证文件头,ico这里验证头是错误的，过滤掉这个 */
					if (!ext.equals("ico")&&!ImageUtils.isImage(file.getInputStream())) {
						throw new GlobalException(
