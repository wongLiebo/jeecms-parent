...
public class ResourcesSpaceDataDaoImpl extends BaseDao<ResourcesSpaceData> implements ResourcesSpaceDataDaoExt {
    ...
    @Override
    public Page<ResourcesSpaceData> getShareList(Integer userId, String alias, String shareUser, Short resourceType,
                                                 Integer resourcesSpaceId, Integer uId, Pageable pageable) {
        JPAQuery<ResourcesSpaceData> jpaQuery = new JPAQuery<>(em);
        QResourcesSpaceData qSpaceData = QResourcesSpaceData.resourcesSpaceData;
        BooleanBuilder builder = new BooleanBuilder();
        ...
        builder.and(qSpaceData.userId.ne(uId));
        ...
        builder.and(qCoreUser.id.eq(uId));
    ...
    public List<ResourcesSpaceData> findBySysPic(Integer querySence) {
        ...
        BooleanBuilder builder = new BooleanBuilder();
        builder.and(qSpaceData.hasDeleted.eq(false));
        if (ResourcesSpaceData.TYPE_SYS_QUERY_VOTE.equals(querySence)) {
            builder.and(qSpaceData.type.in(ResourcesSpaceData.TYPE_VOTE_HEAD, ResourcesSpaceData.TYPE_VOTE_BACKGROUND));
        } else if (ResourcesSpaceData.TYPE_SYS_QUERY_FORM.equals(querySence)) {
            builder.and(qSpaceData.type.in(ResourcesSpaceData.TYPE_FORM_HEAD, ResourcesSpaceData.TYPE_FORM_BACKGROUND));
        }
        return jpaQuery.from(qSpaceData).where(builder).setHint(QueryHints.HINT_CACHEABLE, true).fetch();
    }
