...
public class ContentStaticPageServiceImpl extends AbstractContentListener implements ContentStaticPageService {
     ...
     *                 GlobalException
     */
    private void createOrDelContent(Channel channel, Integer contentCount, boolean containChild, boolean deleteJob,
        ...
        if (channelProcessResult == null) {
            ...
            HashMap<Integer, PageProcessResult> channelProcessResultMap = new HashMap<>();
        ...
        GlobalConfigDTO config = globalConfigService.filterConfig(site, globalConfig, null, null);
        ...
        for (int i = 0; i < loop; i++) {
            ...
            List<Content> contents = contentGetService.getList(
                    new ContentSearchDto(siteId, cids, ContentConstant.ORDER_TYPE_RELEASE_TIME_DESC,
                            Collections.singletonList(ContentConstant.STATUS_PUBLISH), false),
                    paginable);
    ...
    @Override
    public void contentRelated(Content content, boolean ignoreException, HttpServletRequest request,
                               HttpServletResponse response) throws GlobalException {
        // 生成内容页
        content(content);
        ...
        CoreUser user = SystemContextUtils.getCoreUser();
        // 自动生成首页页
        if (site.getConfig().getStaticAutoIndex()) {
    ...
    @Override
    public void deleteContentRelated(Content content, boolean ignoreException, HttpServletRequest request,
                                     HttpServletResponse response) throws GlobalException {
     ...
     */
    @Override
    public void deleteContent(Integer siteId, List<Integer> channelIds, HttpServletRequest request,
                              HttpServletResponse response) throws GlobalException {
        long time = System.currentTimeMillis();
    ...
    private int contentPage(boolean delete, Integer siteId, List<Integer> channelIds, boolean ignoreException,
         ...
         */
        List<Channel> bottomChannels = new ArrayList<>();
        for (Channel c : channels) {
            if (c.getIsBottom()) {
                bottomChannels.add(c);
            }
        }
        Map<Integer, Integer> channelContentCountMap = new HashMap<>();
        HashMap<Integer, PageProcessResult> channelProcessResultMap = new HashMap<>();
    ...
    private void createContentPage(CmsSite site, GlobalConfigDTO config, Map<String, Object> data, Content c,
        ...
        Page<Integer> page = new PageImpl<>(Collections.singletonList(pageNo), pageable, c.getPageCount());
    ...
    public int channel(Integer siteId, List<Integer> channelIds, boolean ignoreException,
        ...
        int count = 0;
        List<Channel> channels = channelService.findAllById(channelIds);
        Map<Integer, Integer> channelPageMap = new HashMap<>();
        Map<Integer, Boolean> preStaticMap = new HashMap<>();
        HashMap<Integer, PageProcessResult> channelProcessResultMap = new HashMap<>();
        CoreUser user = SystemContextUtils.getCoreUser();
        for (Channel c : channels) {
            int channelPage = getChannelPageCount(c, false);
    ...
    private int createOrDelChannel(Channel channel, Boolean preStaticChannel, Integer totalPage, boolean containChild, boolean deleteJob,
        ...
        if (!deleteJob && !StringUtils.isBlank(channel.getLink()) || !site.getOpenStatic()) {
            return 0;
        }
        // 若是空则需要查询栏目页面数量
        if (totalPage == null) {
     ...
     */
    @Override
    public void deleteChannel(Integer siteId, List<Integer> channelIds, boolean ignoreException,
                              HttpServletRequest request, HttpServletResponse response) throws GlobalException {
        long time = System.currentTimeMillis();
        int count = 0;
        List<Channel> channels = channelService.findAllById(channelIds);
     ...
     */
    @Override
    public void deleteIndex(CmsSite site) throws GlobalException {
        File pcIndexFile;
        /** 删除pc和手机端首页 */
    ...
    public void afterContentRecycle(List<Integer> contentIds) throws GlobalException {
        ...
        ThreadPoolService.getInstance().execute(() -> {
            // 发布若是生成错误则继续，不往外抛出异常
            List<Content> contents = contentService.findAllByIdForCache(contentIds);
            try {
                Thread.sleep(2000);
