...
public class CmsModelServiceImpl extends BaseServiceImpl<CmsModel, CmsModelDao, Integer> implements CmsModelService {
    ...
    public enum FilterModel {
        ...
        ALL,
        /**
         * 栏目及内容模型的字段中过滤掉有些特殊字段需要过滤
         */
        SHOW_CHANNEL_AND_CONTENT,
        /**
         * 会员模型的字段中过滤设置 “应用到会员注册” 字段
         */
        SHOW_MEMBER_REGISTOR
    }
    ...
    @Override
    public CmsModel getInfo(Integer id) {
    ...
    @Override
    public List<CmsModel> findModel(Short type) {
        return dao.findByTplTypeAndHasDeleted(type, false);
    }
     ...
     */
    private CmsModel getCmsModelByCondition(Integer id, FilterModel filterModel) {
        CmsModel model = null;
        if (null != id) {
        ...
        List<String> groupTypes = new ArrayList<>(0);
        // 根据不同模型类型，初始化默认字段分组数据
        if (CmsModel.CHANNEL_TYPE.equals(model.getTplType())) {
            groupTypes = new ArrayList<>(Arrays.asList(CmsModelItem.CHANNEL_GROUP_TYPE));
        } else if (CmsModel.CONTENT_TYPE.equals(model.getTplType())) {
            groupTypes = new ArrayList<>(Arrays.asList(CmsModelItem.CONTENT_GROUP_TYPE));
        } else if (CmsModel.MEMBER_TYPE.equals(model.getTplType())) {
            groupTypes = new ArrayList<>(Arrays.asList(CmsModelItem.MEMBER_GROUP_TYPE));
        }
            ...
            if (CmsModel.CONTENT_TYPE.equals(model.getTplType())) {
                ...
                if (CollectionUtils.isEmpty(notBlackItems)) {
                    ...
                    if (!CollectionUtils.isEmpty(dataTypeItems)) {
                        ...
                        boolean isImageUpload = dataTypeMaps.containsKey(CmsModelConstant.SINGLE_CHART_UPLOAD);
