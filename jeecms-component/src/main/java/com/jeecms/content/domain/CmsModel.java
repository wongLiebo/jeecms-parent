...
public class CmsModel extends AbstractSortDomain<Integer> implements Serializable, IBaseSite {
		...
		public JSONArray getUnEnableJson() {
			if(StringUtils.isNoneBlank(this.getUnEnableJsonStr())) {
			        return JSON.parseArray(this.getUnEnableJsonStr());
            }
			return unEnableJson;
		}
    ...
    public static final Short CONTENT_TYPE = 2;
        ...
        public String getModelField() {
            ...
            if (!CollectionUtils.isEmpty(items)) {
                return StringUtils.join(items.stream().map(CmsModelItem::getField).collect(Collectors.toList()));
            }
            return null;
        }
}
