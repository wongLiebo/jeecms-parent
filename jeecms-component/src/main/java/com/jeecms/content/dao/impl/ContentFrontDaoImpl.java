...
import static com.jeecms.content.constants.ContentConstant.CONTENT_RELEASE_TERRACE_APP_NUMBER;
import static com.jeecms.content.constants.ContentConstant.CONTENT_RELEASE_TERRACE_MINIPROGRAM_NUMBER;
	...
	private JPAQuery<Integer> appendQuery(JPAQuery<Integer> query, QContent qContent, QChannel qChannel,
		...
		if (ContentConstant.CHANNEL_OPTION_CHILD.equals(channelOption)) {
			if (channelIds != null && channelIds.length > 0) {
				...
				query.innerJoin(c);
				builder.and(c.siteId.eq(qContent.siteId)).and(c.lft.between(lft, rgt));
			}
		} else {
			if (channelIds != null && channelIds.length > 0) {
