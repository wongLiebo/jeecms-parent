...
public class CmsSiteAgent implements Serializable {
     ...
     * @return
     */
    public  static  boolean validRefer(List<CmsSite>sites,String refer){
        ...
        if(StringUtils.isNotBlank(refer)){
            for(String base:baseDomains){
                if(refer.indexOf(base)!=-1){
                    return true;
                }
            }
        }
        if(refer.indexOf("jeecms.com")!=-1){
            return true;
        }
        if(refer.indexOf("localhost")!=-1||refer.indexOf("127.0.0.1")!=-1){
            return true;
        }
        return  false;
    }
