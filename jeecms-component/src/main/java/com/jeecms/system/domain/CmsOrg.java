...
public class CmsOrg extends com.jeecms.common.base.domain.AbstractTreeDomain<CmsOrg, Integer> implements Serializable {
        ...
        public Set<CmsSite> getSiteOfOwnerDataPermsByType(Short type) {
                CopyOnWriteArraySet<CmsDataPerm> ownerPerms = getAllOwnerDataPermsByType(type);
                Set<CmsSite> ownerDataPermsSite = new HashSet<>();
        ...
        public CopyOnWriteArraySet<CmsDataPerm> getAllOwnerDataPermsByType(Short type) {
                ...
                for(CmsSite site:sites){
                        set.addAll(getOwnerDataPermsByType(type,site.getId(),null,null,true));
                }
                return set;
        }

        @Transient
        public Set<CmsSite> getAllOwnerSiteByDataPermType(Short type) {
                CopyOnWriteArraySet<CmsDataPerm> set = new CopyOnWriteArraySet<CmsDataPerm>();
                Set<CmsSite> sites = getOwnerSites();
                Set<CmsSite> hasPermSites = new HashSet<>();
                for(CmsSite site:sites){
                        Set<CmsDataPerm> ownerDataPerms = getOwnerDataPermsByType(type,site.getId(),null,null,true);
                        if(!ownerDataPerms.isEmpty()){
                               hasPermSites.add(site);
                       }
                }
                return hasPermSites;
        }
...
//                                                containEmptyChannelSite);
                        ...
                        if (dataPerms != null && dataPerms.size() > 0) {
                                if (StringUtils.isNoneBlank(cacheKey)) {
                                        cache.setCache(cacheKey,getCacheId(type,siteId), (Serializable) dataPerms);
                                }
                        }
                ...
                if (!getHasAssignDataPermsByType(type)) {
                        CmsOrg parent = getParent();
                        while (parent != null) {
                                perms = parent.getOwnerDataPermsByType(type, siteId, null, null, containEmptyChannelSite);
                ...
                /**第三层节点以后节点才需要过滤，第二次继承自顶层拥有所有权限*/
                if (getParent()!=null&&!getParent().isTop()) {
...
//                }
        ...
        private Set<CmsDataPerm> filterOwnerDataPermsByType(Set<CmsDataPerm> ownerPerms,
                ...
                if (getParent() != null) {
                        Set<CmsDataPerm> parentOwnerPerms =  getParent().getOwnerDataPermsByType(type,
                                siteId, operator, null, true);
                        Set<CmsDataPerm> filterPerms = new HashSet<>();
...
//                }
        ...
        public Set<CmsOrg> getChild() {
                return child;
        }

        public void setChild(Set<CmsOrg> child) {
                this.child = child;
        }
