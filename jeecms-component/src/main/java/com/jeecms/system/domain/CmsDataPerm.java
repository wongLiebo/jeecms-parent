...
public class CmsDataPerm extends com.jeecms.common.base.domain.AbstractDomain<Integer>
         ...
         * @return: String
         */
        public static final String getOpeSites() {
                StringBuffer buff = new StringBuffer();
                for (Short o : OPE_SITE_ARRAY) {
         ...
         * 内容浏览记录 16
         */
        public static final Short OPE_CONTENT_BROWSING_HISTORY = 16;
        ...
        public static final Short[] OPE_CONTENT_ARRAY = new Short[] { OPE_CONTENT_VIEW, OPE_CONTENT_CREATE,
                        ...
                        OPE_CONTENT_SITE_PUSH, OPE_CONTENT_WEIBO_PUSH, OPE_CONTENT_WECHAT_PUSH, OPE_CONTENT_BROWSING_HISTORY };
        ...
        public static Set<CmsDataPerm> streamFilter(Set<CmsDataPerm> perms,
                ...
                Set<CmsDataPerm> set = new HashSet<>();
                set.addAll(permSet);
                return set;
        }

        public static  void initPermDataChannel(Set<CmsDataPerm> perms, Short type){
                ChannelService channelService = ApplicationContextProvider.getBean(ChannelService.class);
                /**其他权限检查地方需要用到dataChannel对象*/
                if (CmsDataPerm.DATA_TYPE_CHANNEL.equals(type) || CmsDataPerm.DATA_TYPE_CONTENT.equals(type)) {
                        for (CmsDataPerm d : perms) {
                                if (d.getDataChannel() == null&&d.getDataId()!=null) {
                                        d.setDataChannel(channelService.findById(d.getDataId()));
                                }
                        }
                }
        }
...
//        @JoinColumn(name = "data_id", insertable = false, updatable = false)
        /**还有站点数据权限，如果此处加上映射，无法加载站点类权限数据的栏目对象，在栏目权限设置特殊处理，调用setDataChannel设置*/
        @Transient
