...
public class CmsSite extends com.jeecms.common.base.domain.AbstractTreeDomain<CmsSite, Integer>
        ...
        /**
         * 返回首页完整前缀(类似/WEB-INF/t/cms/www/default/index/index)
         *
         * @param solutionPath
         *                模板方案名
         * @return 完整前缀
         ...
         * @return 完整前缀
         */
        public String getTplPrefix(String solutionPath, String prefix) {
                StringBuilder t = new StringBuilder();
                t.append(getTplPath()).append(SPT);
                t.append(solutionPath).append(SPT);
                t.append(prefix).append(SPT);
                return t.toString();
        }
         ...
         */
        @Transient
        public List<Integer> getChildAllId() {
            List<CmsSite> sites = getChildAll();
            List<Integer> ids = sites.stream().filter(x->!x.getIsDelete()).map(CmsSite::getId).collect(Collectors.toList());
            return ids;
        }
        ...
        public List<Integer> getChildIdList() {
            List<CmsSite> list = new ArrayList<>(getChildAll());
            if (CollectionUtils.isEmpty(list) || list.size()<2) {
                return new ArrayList<>();
            }
            List<Integer> ids = list.stream().filter(x->!x.getId().equals(this.id) && !x.getIsDelete()).map(CmsSite::getId).collect(Collectors.toList());
            return ids;
        }

        @Transient
        public List<CmsSite> getChildAllList() {
            List<CmsSite> list = new ArrayList<>(getChildAll());
            if (CollectionUtils.isEmpty(list) || list.size()<2) {
                return new ArrayList<>();
            }
            list = list.stream().filter(x->!x.getId().equals(this.id) && !x.getIsDelete()).collect(Collectors.toList());
            return list;
        }
        ...
        public boolean isAllowPicMaxFile(int size) {
                Double allowPerFile = getUploadPicSize();
                if (null == allowPerFile || allowPerFile<=0) {
                        return true;
                } else {
        ...
        public boolean isAllowAttachmentMaxFile(int size) {
                Double allowPerFile = getUploadAttachmentSize();
                if (null == allowPerFile || allowPerFile<=0) {
                        return true;
                } else {
                        return allowPerFile >= size;
        ...
        public boolean isAllowAudioMaxFile(int size) {
                Double allowPerFile = getUploadAudioSize();
                if (null == allowPerFile || allowPerFile<=0) {
                        return true;
                } else {
                        return allowPerFile >= size;
        ...
        public boolean isAllowVideoMaxFile(int size) {
                Double allowPerFile = getUploadVideoSize();
                if (null == allowPerFile || allowPerFile<=0) {
                        return true;
                } else {
                        return allowPerFile >= size;
        ...
        @Transient
        public Double getUploadPicSize() {
                Double uploadMaxSize = this.getCmsSiteCfg().getUploadPicSize();
        ...
        public Double getUploadVideoSize() {
                Double uploadMaxSize = this.getCmsSiteCfg().getUploadVideoSize();
        ...
        @Transient
        public Double getUploadAttachmentSize() {
                Double uploadMaxSize = this.getCmsSiteCfg().getUploadAttachmentSize();
        ...
        @Transient
        public Double getUploadAudioSize() {
                Double uploadMaxSize = this.getCmsSiteCfg().getUploadAudioSize();
        ...
        @Transient
        public Double getAllowPicFileMaxSize() {
         ...
         */
        @Transient
        public List<Channel>getNoReCycleChannels(){
                return getChannels().stream().filter(s->!s.getRecycle()).collect(Collectors.toList());
        }
