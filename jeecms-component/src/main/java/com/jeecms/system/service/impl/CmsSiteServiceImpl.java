...
public class CmsSiteServiceImpl extends BaseServiceImpl<CmsSite, CmsSiteDao, Integer> implements CmsSiteService,
	...
	public CmsSite save(CmsSite currSite, CoreUser currUser, CmsSite bean) throws GlobalException {
		...
		Map<String, String> map = new HashMap<>(parentMap);
		/** 继承父站点扩展 **/
		bean.setCfg(map);
		/** 初始化站点Ext **/
		...
		parent.getChilds().add(beans);
		super.flush();
		/**优先清空当前用户的权限，否则加载站点树接口比子线程处理权限缓存执行要快，加载不到最新的权限站点数据*/
		if(currUser.getHasAssignDataPermsByType(CmsDataPerm.DATA_TYPE_SITE)){
			currUser.clearPermCache();
			/**用户权限需要过滤非组织角色的权限，则此处也需要清空所属组织和角色的权限*/
			for(CoreRole role:currUser.getRoles()){
				role.clearPermCache();
			}
			currUser.getOrg().clearPermCache();
			dataPermService.updateUserSite(currUser,beans);
		}
		 ...
		 */
		try {
			cmsSiteExtService.saveDir(bean.getParentId(), beans);
		} catch (IOException e) {
...
//	}
	...
	public CmsSite getCurrSite(HttpServletRequest request, HttpServletResponse response) throws GlobalException {
		...
		if (site == null) {
			...
			/**通过参数或者cookie获取到的站点，如果当前是管理员登录并且没有该站点权限，则重新定位到管理员有权限的站点*/
	...
	@Override
	public CmsSite getByCookie(HttpServletRequest request) throws GlobalException {
	...
	public void afterPropertiesSet() throws Exception {
		ThreadPoolService.getInstance().execute(() -> {
			for(CmsSite site:findAll(false,true)){
				site.getChannels().size();
			}
		});
	}
