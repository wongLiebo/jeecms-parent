...
import com.jeecms.system.domain.vo.*;
...
public class CmsDataPermPlusServiceImpl implements CmsDataPermPlusService {
        ...
        @Override
        public CmsDataPermVo assembleData(Integer orgId, Integer roleId, Integer userId, Integer siteId, Short dataType, CoreUser currUser)
                        throws GlobalException {
                CmsDataPermVo vo = new CmsDataPermVo();
                /** 权限来源组织 */
                CmsOrg sourceOrg = null;
                if (CmsDataPerm.DATA_TYPE_SITE_OWNER.equals(dataType)) {
                        List<SiteMap> siteDatas = new ArrayList<>();
                        /** 展示站点 */
                        CopyOnWriteArraySet<CmsSite> viewSites = new CopyOnWriteArraySet<>();
                        /** 组织/角色/用户拥有的站群站点 */
                        HashSet<CmsSite> ownerSites = new HashSet<>();
                        /** 当前登录用户可分配的站点 */
                        Set<CmsSite> canAssignSites = new CopyOnWriteArraySet<>();
                        canAssignSites.addAll(currUser.getOwnerSites());
                        CoreUserAgent.agentOwnerSites(currUser);
                        ...
                        if (orgId != null) {
                                ...
                                if (org != null) {
                                        if (org.getParent() != null) {
                                                ...
                                                CmsOrgAgent.agentOwnerSites(org.getParent());
                                ...
                                if (role != null) {
                                        ...
                                        CmsOrgAgent.agentOwnerSites(role.getOrg());
                                        ...
                                        //可分配站点还需要过滤掉所属组织所有的站群权限
                                        List<Integer> orgOwnerSiteIds = CmsSite.fetchIds(role.getOrg().getOwnerSites());
                                        for(CmsSite s:canAssignSites){
                                                if(!orgOwnerSiteIds.contains(s.getId())){
                                                        canAssignSites.remove(s);
                                                }
                                        }
                                }
                        } else if (userId != null) {
                                ...
                                if (user != null) {
                                       //viewSites = user.getOrg().getOwnerSites();
                                        CmsOrgAgent.agentOwnerSites(user.getOrg());
                                        ownerSites.addAll(user.getOwnerSites());
                                        CoreUserAgent.agentOwnerSites(user);
                                        ...
                                        //可分配站点还需要过滤掉所属组织所有的站群权限
                                        List<Integer> orgOwnerSiteIds = CmsSite.fetchIds(user.getOrg().getOwnerSites());
                                        for(CmsSite s:canAssignSites){
                                                if(!orgOwnerSiteIds.contains(s.getId())){
                                                        canAssignSites.remove(s);
                                                }
                        ...
                        /** 展示站点权限数据 */
                        Set<CmsDataPerm> viewSiteDataPerms = new HashSet<>();
                        /** 组织/角色/用户拥有的站点权限数据 */
                        CopyOnWriteArraySet<CmsDataPerm> ownerSiteDataPerms = new CopyOnWriteArraySet<>();
                        List<Short> ownerMewSiteOpes = new ArrayList<>();
                        /** 当前登录用户可分配的站点权限数据 */
                        CopyOnWriteArraySet<CmsDataPerm> canAssignDataPerms = new CopyOnWriteArraySet<>();
                        List<Short> canAssignNewSiteOpes = new ArrayList<>();
                        ...
                        permTree.add(getNewSiteRow(org, ownerMewSiteOpes, canAssignNewSiteOpes));
                        vo.setDataIds(permTree);
                } else {
                        /** 栏目、内容、数据权限 */
                        /** 展示栏目类权限数据 */
                        Set<CmsDataPerm> viewDataPerms = new HashSet<>();
                        /** 组织/角色/用户拥有的栏目类权限数据 */
                        ...
                        /** 是否可分配数据权限需要综合上级设置 */
                        if (sourceOrg != null) {
                                Set<CmsDataPerm> sourceDataPerms = sourceOrg
                                                .getOwnerDataPermsByType(dataType, siteId, null, null, true);
                                CopyOnWriteArraySet<CmsDataPerm> filterAssignDataPerms = new CopyOnWriteArraySet<>();
        ...
        public Set<CmsSite> getSites(Integer orgId, Integer roleId, Integer userId, Short dataType) {
                ...
                CoreRole role;
                /** 上级组织站群类权限站点 */
                Set<CmsSite> ownerSites = new CopyOnWriteArraySet<>();
                /** 上级组织、所属组织 文档或栏目权限站点 */
                Set<CmsSite> orgDataSites;
                ...
                if (originOrg != null) {
                        orgDataSites = originOrg.getAllOwnerSiteByDataPermType(dataType);
        ...
        public List<OrgPermVo> assembleDataByChannelForOrg(Channel channel, CmsOrg org, Short dateType) {
                ...
                if (channel != null) {
                        ...
                        /** 当前登录用户可分配的栏目类权限数据 */
                        Set<CmsDataPerm> canAssignPerms = currUser.getOwnerDataPermsByType(dateType,
                        ...
                        for (CmsOrg or : orgs) {
                                ...
                                orgPermVo.setOrg(or);
                                /** 组织/角色/用户拥有的栏目类权限数据 */
                                Set<CmsDataPerm> orgOwnerPerms = or.getOwnerDataPermsByType(dateType,
                                                siteId, null, channel.getId(), false);
                                Set<CmsDataPerm> parentOrgPerms  = orgOwnerPerms;
                                if(or.getParent()!=null){
                                        parentOrgPerms = or.getParent().getOwnerDataPermsByType(dateType,
                                                siteId, null, channel.getId(), false);
                                }
                                CmsDataPermVo.ChannelUnit channelUnit = getChannelUnit(or, null, null, channel,
                                                currUser, dateType, orgOwnerPerms, canAssignPerms,parentOrgPerms);
                                /** 先添加组织的权限信息 */
                                ...
                                for (CoreRole role : or.getRoles()) {
                                        ...
                                        /** 组织/角色/用户拥有的栏目类权限数据 */
                                        Set<CmsDataPerm> roleOwnerPerms = role.getOwnerDataPermsByType(
                                                        dateType, siteId, null, channel.getId());
                                        CmsDataPermVo.ChannelUnit roleChannelUnit = getChannelUnit(null, role, null,
                                                        channel, currUser, dateType, roleOwnerPerms, canAssignPerms,orgOwnerPerms);
        ...
        public Page<UserPermVo> assembleDataByChannelForUser(Channel channel, CmsOrg org, Short dateType, Integer orgid,
                ...
                if (channel != null) {
                        ...
                        for (CoreUser user : users.getContent()) {
                                ...
                                /** 组织/角色/用户拥有的栏目类权限数据 */
                                Set<CmsDataPerm> userOwnerPerms = user.getOwnerDataPermsByType(dateType,
                                                siteId, null, channel.getId());
                                Set<CmsDataPerm> orgOwnerPerms  = user.getOrg().getOwnerDataPermsByType(dateType,siteId,null,channel.getId(),false);
                                CmsDataPermVo.ChannelUnit channelUnit = getChannelUnit(null, null, user, channel,
                                                currUser, dateType, userOwnerPerms, canAssignPerms,orgOwnerPerms);
        ...
        public List<OrgOwnerSitePermVo> assembleDataByOwnerSiteForOrg(CmsSite site, CmsOrg org) {
                ...
                for (CmsOrg or : orgs) {
                        ...
                        CmsOrgAgent.agentOwnerSites(or);
                        /** 原分配站群权限数据 */
        ...
        public List<OrgPermVo> assembleDataBySiteForOrg(CmsSite site, CmsOrg org) {
                ...
                CoreUser currUser = SystemContextUtils.getUser(RequestUtils.getHttpServletRequest());
                /** 当前登录用户可分配的栏目类权限数据 */
                Set<CmsDataPerm> canAssignPerms = currUser
                ...
                for (CmsOrg or : orgs) {
                        ...
                        orgPermVo.setOrg(or);
                        /** 组织/角色/用户拥有的栏目类权限数据 */
                        Set<CmsDataPerm> orgOwnerPerms = or.getOwnerDataPermsByType(
                                        CmsDataPerm.DATA_TYPE_SITE, site.getId(), null,site.getId(), false);
                        Set<CmsDataPerm> parentPerms = orgOwnerPerms;
                        if(or.getParent()!=null){
                                parentPerms = or.getParent().getOwnerDataPermsByType(
                                        CmsDataPerm.DATA_TYPE_SITE, site.getId(), null,site.getId(), false);
                        }
                        List<MiniDataUnit> siteOps = getInitSiteOps(or, null, null, currUser, orgOwnerPerms,
                                        canAssignPerms,parentPerms);
                        /** 先添加组织的权限信息 */
        ...
        public Page<UserPermVo> assembleDataBySiteForUser(CmsSite site, CmsOrg org, Integer orgid, Integer roleid,
               ...
               Set<CmsDataPerm> canAssignPerms = currUser
                ...
                for (CoreUser user : users.getContent()) {
                        ...
                        userPermVo.setUser(user);
                        /** 组织/角色/用户拥有的站点类权限数据 */
                        Set<CmsDataPerm> ownerPerms = user
                                        .getOwnerDataPermsByType(CmsDataPerm.DATA_TYPE_SITE, null, null, site.getId());
                        Set<CmsDataPerm> orgPerms = user.getOrg().getOwnerDataPermsByType(CmsDataPerm.DATA_TYPE_SITE, site.getId(), null, null,true);
                        userPermVo.setOps(getInitSiteOps(null, null, user, currUser, ownerPerms, canAssignPerms,orgPerms));
                        userPermVos.add(userPermVo);
                }
                Page<UserPermVo> page = new PageImpl<>(userPermVos, pageable, users.getTotalElements());
                return page;
        ...
        private ChannelRow createOneSiteChannelRows(CmsSite site, CoreUser currUser, Short dataType,
                ...
                ChannelRow oneSiteData = new ChannelRow();
                ...
                for (Channel c : toViewChannels) {
                        ...
                        if (channelIds.contains(c.getId())) {
                                ...
                                units.add(getChannelUnit(org, role, user, c, currUser, dataType, ownerSiteDataPerms,
                                                canAssignSiteDataPerms,canAssignSiteDataPerms));
                        }
                }
                /** 新栏目配置 */
        ...
        private ChannelDataPermTree getNewSiteChannelRows(CmsOrg org, Short dataType, Collection<Short> viewChannelOpes,
                ...
                List<MiniDataUnit> rowOpes = new ArrayList<MiniDataUnit>();
                ...
                for (Short p : opes) {
                        ...
                        MiniDataUnit miniUnit = new MiniDataUnit(p, canAssign, hasPerm);
        ...
        private CmsDataPermVo.ChannelUnit getChannelUnit(CmsOrg org, CoreRole role, CoreUser user, Channel channel,
                        ...
                        Set<CmsDataPerm> canAssignDataPerms,Set<CmsDataPerm> orgOwnerPerms) {
                ...
                List<Short> canAssignOps = canAssignDataPerms.stream().map(perm -> perm.getOperation())
                        .collect(Collectors.toList());
                List<Short> orgCanAssignOps = orgOwnerPerms.stream().map(perm -> perm.getOperation())
                        .collect(Collectors.toList());
                for (Short p : ops) {
                        boolean canAssign = getCanAssignInitValue();
                        boolean hasPerm = false;
        ...
        private List<CmsDataPermVo.SiteRow> getSiteRow(CmsOrg org, Set<CmsDataPerm> viewDataPerms,
                ...
                for (Integer siteId : siteIdList) {
                        ...
                        List<MiniDataUnit> units = new ArrayList<MiniDataUnit>();
                        ...
                        for (CmsDataPerm p : viewDataPermMap.get(siteId)) {
                                ...
                                MiniDataUnit unit = new MiniDataUnit(p.getOperation(),
                                                canAssign, hasPerm);
                        ...
                        for (Short op : CmsDataPerm.OPE_SITE_ARRAY) {
                                if (!hasViewOpes.contains(op)) {
                                        MiniDataUnit unit = new MiniDataUnit(op, false,
                                                        false);
                                        siteRow.getRowDatas().add(unit);
                                }
                        }
        ...
        private List<MiniDataUnit> getInitSiteOps(CmsOrg org, CoreRole role, CoreUser user, CoreUser currUser,
                        Set<CmsDataPerm> ownerDataPerms, Set<CmsDataPerm> canAssignDataPerms,Set<CmsDataPerm> orgPerms) {
                List<MiniDataUnit> units = new ArrayList<MiniDataUnit>();
                for (Short p : CmsDataPerm.OPE_SITE_ARRAY) {
                        ...
                        /**用户、角色所属组织没有权限则不可分配*/
                        if(!CmsDataPerm.getOperators(orgPerms).contains(p)){
                                canAssign = false;
                        }
                        /** 顶层组织不可分配权限 */
                        ...
                        MiniDataUnit unit = new MiniDataUnit(p, canAssign, hasPerm);
                        units.add(unit);
                }
                return units;
        }
        ...
        private SiteMap getSiteMap(CmsOrg org, CoreRole role, CoreUser user, CoreUser currUser,
                        CmsSite site, Set<CmsSite> ownerSites, Set<CmsSite> canAssignSites) {
                SiteMap siteRow = new SiteMap();
                ...
                Set<CmsSite> orgCanAssignSites = canAssignSites;
                if (user!=null){
                        orgCanAssignSites = user.getOrg().getOwnerSites();
                }
                if (role!=null){
                        orgCanAssignSites = role.getOrg().getOwnerSites();
                }
                if(org!=null&&org.getParent()!=null){
                        orgCanAssignSites = org.getParent().getOwnerSites();
                }
                ...
                /**所属组织或上级组织可分配站群权限如果不包含该站点则不可分配*/
                if (!CmsSite.fetchIds(orgCanAssignSites).contains(site.getId())){
