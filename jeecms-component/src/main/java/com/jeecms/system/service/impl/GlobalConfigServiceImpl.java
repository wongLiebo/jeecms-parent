    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
            ...
            ThreadPoolService.getInstance().execute(new Runnable() {
                    ...
                    public void run() {
                            ...
                            List<CmsDataPermCfg>permCfgs = permCfgService.findAll(true);
                            List<CmsDataPerm>perms =permService.findAll(true);
                            try {
                            ...
                            if(permCfgs!=null&&permCfgs.size()>0){
                                    logger.debug(permCfgs.stream().findFirst().get().getCreateUser());
                            }
                            if(perms!=null&&perms.size()>0){
                                    logger.debug(perms.stream().findFirst().get().getCreateUser());
                            }
                    }
            });
        }
