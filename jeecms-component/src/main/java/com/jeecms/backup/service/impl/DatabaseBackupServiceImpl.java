...
public class DatabaseBackupServiceImpl implements DatabaseBackupService {
	...
	public ResponseInfo backup(Integer siteId, String remark) throws GlobalException {
		...
		if (!validDbDriverClass()) {
			throw new GlobalException(new SystemExceptionInfo(OtherErrorCodeEnum.BACK_DB_NO_SUPPORT_ERROR.getDefaultMessage()
					,OtherErrorCodeEnum.BACK_DB_NO_SUPPORT_ERROR.getCode()));
		}
...
//        contentMap.put("password", "123456");
	...
	public void onBackupComplete(CompleteDto dto) throws Exception {
		...
		if (dto.getSuccess()) {
			...
			String url = String.format("http://%s:%d/jeecms-backup/download?filepath=%s", dataSourceMap.get("host"), DB_BACKUP_SERVER_PORT, dto.getBakFilePath());
			//从备份服务器获取备份文件
			file =  HttpClientUtil.getFile(url, realPathResolver.get(UPLOADPATH + WebConstants.SPT + FilenameUtils.getName(dto.getBakFilePath())));
			if (file.exists()) {
				// 获取文件后缀
	...
	@Override
	public void deleteBackup(List<DatabaseBackupRecord> list) throws Exception {
		//先判断是否存在备份中和还原中的数据文件
		for (DatabaseBackupRecord backupRecord : list) {
			if (backupRecord.getState() == DatabaseBackupRecord.IN_BACKUP) {
				throw new GlobalException(new IllegalParamExceptionInfo("正在备份中, 无法删除", null));
			} else if (backupRecord.getState() == DatabaseBackupRecord.IN_RECOVERY) {
				throw new GlobalException(new IllegalParamExceptionInfo("正在还原中, 无法删除", null));
		...
		//删除记录
		databaseBackupRecordService.physicalDeleteInBatch(list);
	}
