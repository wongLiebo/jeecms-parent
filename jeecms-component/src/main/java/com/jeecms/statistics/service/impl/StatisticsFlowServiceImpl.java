...
import com.jeecms.statistics.domain.vo.*;
...
import static com.jeecms.statistics.domain.dto.StatisticsDto.*;
...
public class StatisticsFlowServiceImpl extends BaseServiceImpl<StatisticsFlow, StatisticsFlowDao, Integer>
	...
	public List<StatisticsFlowRealTimeVo> getRealTimeVo(Integer siteId) {
		...
		// 时间和操作记录集合
		Map<Date, List<SysAccessRecord>> dateMap = new LinkedHashMap<>();
		List<SysAccessRecord> accessRecords = sysAccessRecordService.findByDate(startTime, endTime, siteId, null);
	...
	public AccessDeviceVo timeDevice(Integer siteID) throws Exception {
		...
		for (Short key : device.keySet()) {
			...
			if (key.equals(SysAccessRecord.COMPUTER)) {
				vi.setType("计算机");
			} else {
				vi.setType("移动设备");
			}
	...
	public AccessDeviceVo device(StatisticsDto dto) throws Exception {
		...
		for (Integer key : device.keySet()) {
			...
			DeviceVo vi = new AccessDeviceVo().new DeviceVo();
			if (key.equals(StatisticsFlow.COMPUTER)) {
				vi.setType("计算机");
			} else {
				vi.setType("移动设备");
			}
...
//		} while (cl3.getTime().getTime() < cl2.getTime().getTime());
	...
	public StatisitcsOverviewVos getOverviewVos(Integer siteId) {
		...
		List<SysAccessRecord> records = sysAccessRecordService.findByDate(startTime, endTime, siteId, null);
		Integer pv = records == null ? 0 : records.size();
		Integer uv = records == null ? 0 : this.receiveUv(records);
		Integer ip = records == null ? 0 : this.receiveIp(records);
		...
		//基于x1.4.0的绩效管理新增网站统计数据
		return new StatisitcsOverviewVos(now, yesterday, highest, sum(siteId));
	}

	/**新增累计**/
	private StatisitcsOverviewVo sum(Integer siteId) {
		int uv = 0;
		List<StatisticsFlow> all = dao.findAll();
		all = all.stream().filter(x -> x.getSiteId().equals(siteId)).collect(Collectors.toList());
		uv = all.stream().mapToInt(StatisticsFlow::getUvs).sum();
		Map<String, String[]> params = new HashMap<>(2);
		params.put("EQ_siteId_Integer", new String[]{siteId.toString()});
		return new StatisitcsOverviewVo(
				(int)sysAccessRecordService.count(params),
				uv,
				(int)sysAccessRecordService.countIp(null, siteId), "--", "--");
	}
