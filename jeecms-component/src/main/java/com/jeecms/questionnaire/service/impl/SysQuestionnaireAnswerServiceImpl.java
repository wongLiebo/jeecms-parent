...
class SysQuestionnaireAnswerServiceImpl extends BaseServiceImpl<SysQuestionnaireAnswer, SysQuestionnaireAnswerDao, Integer> implements SysQuestionnaireAnswerService {
	...
	public QuestionnaireAnswerViewVo getById(Integer id) {
		...
		List<SysQuestionnaireAnswer> list = dao.findByReplayNameAndQuestionnaireId(
			answer.getReplayName(), answer.getQuestionnaireId(), answer.getUuid());
	...
	public List<SysQuestionnaireAnswer> markStatus(Integer[] ids, Boolean status) throws GlobalException {
		...
		List<String> uuids = new ArrayList<String>(list.size());
		for (SysQuestionnaireAnswer answer : list) {
			if (!answer.getIsEffective().equals(status)) {
				uuids.add(answer.getUuid());
			}
		}
		List<SysQuestionnaireAnswer> answers = dao.find(uuids);
		for (SysQuestionnaireAnswer answer : answers) {
			if (!answer.getIsEffective().equals(status)) {
	...
	public void deleteByIds(Integer[] ids) throws GlobalException {
		...
		for (SysQuestionnaireAnswer answer : list) {
			...
			Integer answerCount = q.getAnswerCount();
			q.setAnswerCount(answerCount > 0 ? answerCount - 1 : 0);
				...
				if (filess.size() > 0) {
					...
					for (MultipartFile file : filess) {
						...
						try {
							...
							if (ImageUtils.isImage(inputStream)) {
							...
							String originalFilename = result.getFileUrl();
							dto.setUrl(originalFilename);
							dto.setName(file.getOriginalFilename());
							// 转换pdf
							dto.setType(result.getResourceType());

							String fullPath = FilenameUtils.getFullPath(originalFilename);
							String pdfUrl = fullPath + FilenameUtils.getBaseName(originalFilename);
							if (file.getSize() > 0) {
								...
								if (ContentUtil.isDocSuffix(ext)) {
									openOfficeConverter.convertToPdf(tempFile.getAbsolutePath(),
										realPathResolver.get("/"), pdfUrl);
									dto.setPdfUrl(pdfUrl + ".pdf");
								}
							}
							...
							if (StringUtils.isBlank(dto.getPdfUrl())) {
								dto.setPdfUrl(dto.getUrl());
							}
							list.add(dto);
	...
	private List<SysQuestionnaireAnswer> getReplayObject(List<SysQuestionnaireAnswer> list) {
		...
		Map<String, List<SysQuestionnaireAnswer>> collect = list.parallelStream()
			.filter(o -> StringUtils.isNotBlank(o.getUuid()))
			.collect(Collectors.groupingBy(SysQuestionnaireAnswer::getUuid));
		for (List<SysQuestionnaireAnswer> value : collect.values()) {
			answerList.add(value.get(0));
		}
		return answerList;
	}
