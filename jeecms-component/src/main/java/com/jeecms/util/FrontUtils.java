...
public class FrontUtils extends FrontUtilBase {
	 ...
	 * @throws TemplateModelException
	 */
	public static MemberVo getUser(Environment env)
			throws TemplateModelException {
		TemplateModel model = env.getGlobalVariable(USER);
		if (model instanceof AdapterTemplateModel) {
			return (MemberVo) ((AdapterTemplateModel) model)
					.getAdaptedObject(MemberVo.class);
		}
		return null;
	}
	...
	private static MemberVo filterMember(CoreUser member) {
		if (member != null) {
			...
			vo.setSecretName(member.getUserSecretName());
			vo.setSecretId(member.getUserSecretId());
			return vo;
		}
		return null;
	}
