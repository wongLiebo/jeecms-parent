...
public class UserCommentPcServiceImpl implements UserCommentPcService {
	...
	public void save(UserCommentSaveDto dto, Content content, HttpServletRequest request, HttpServletResponse response) throws GlobalException {
		...
		userComment.setChannelId(content.getChannelId());
		...
		//加入地区
		userComment.setVisitorArea(userCommentService.getVersionArea(ip));
		...
		service.flush();
		// 只有在审核通过的前提下才会加入一个值
		if (UserCommentConstan.CHECK_BY.equals(userComment.getStatus())) {
			if (userComment.getUserId() != null) {
		...
		if (userComment.getUserId() != null) {
			cacheProvider.setCache(UserCommentConstan.USER_COMMENT_CACHE_KEY, String.valueOf(userComment.getUserId()), currentTime);
		} else {
			sessionProvider.setAttribute(request, UserCommentConstan.USER_COMMENT_TIME_INTERVAL, currentTime);
		}
	}
	...
	private String checkConfig(UserCommentSaveDto dto, CmsSite site, Long currentTime,
		...
		if (dto.getUserId() != null) {
			if (cacheProvider.exist(UserCommentConstan.USER_COMMENT_CACHE_KEY, String.valueOf(dto.getUserId()))) {
				oldTime = Long.valueOf(String.valueOf(cacheProvider.getCache(UserCommentConstan.USER_COMMENT_CACHE_KEY, String.valueOf(dto.getUserId()))));
			}
		} else {
			String oldTimeStr = String.valueOf(sessionProvider.getAttribute(request, UserCommentConstan.USER_COMMENT_TIME_INTERVAL));
			...
			for (UserComment userComment : commentPage) {
				...
				if (commentChildren != null && commentChildren.size() > 0) {
					...
					commentChildren = commentChildren.stream()
						.filter(comment -> UserCommentConstan.CHECK_BY.equals(comment.getStatus()))
	...
	public UserComment getComment(Integer id, CoreUser user, HttpServletRequest request, Integer siteId) throws GlobalException {
		...
		if (commentChildren != null && commentChildren.size() > 0) {
			// 排除子集中审核不通过的子集
			commentChildren = commentChildren.stream()
				.filter(comment -> UserCommentConstan.CHECK_BY.equals(comment.getStatus())).collect(Collectors.toList());
			userComment.setChildren(commentChildren);
			for (UserComment children : commentChildren) {
				children = this.processFunction(children, user, likeCommentIds, reportCommentIds, request);
			}
	...
	private Boolean isLike(Integer commentId, HttpServletRequest request) {
		...
		if (userId == null) {
			...
			List<Integer> commentIds = getCommentIds(true, user);
	...
	public void like(CoreUser user, Integer commentId, HttpServletRequest request,
		...
		Element element = cache.get(commentId);
		UserComment comment;
		if (element == null) {
			comment = service.findById(commentId);
		} else {
			comment = (UserComment) element.getObjectValue();
		}
		if (user != null) {
	...
	private void saveToDB(Ehcache cache) throws GlobalException {
		...
		List<UserComment> comment = new ArrayList<>(keySize);
		Element element;
		for (Integer it : keys) {
			element = cache.get(it);
			if (element != null) {
				comment.add((UserComment) element.getObjectValue());
			}
		}
		service.batchUpdate(comment);
		service.flush();
	}
