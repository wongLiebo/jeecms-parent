...
import com.jeecms.wechat.domain.*;
import com.jeecms.wechat.domain.vo.WechatFansVO;
import com.jeecms.wechat.service.*;
...
public class WechatFansServiceImpl extends BaseServiceImpl<WechatFans, WechatFansDao, Integer>
	...
	public ResponseInfo getFansPage(List<String> appids, Pageable pageable, String nickname, 
		...
		List<AbstractWeChatInfo> infos = new ArrayList<>();
		//查询公众号信息
		for (String appid : appids) {
			infos.add(abstractWeChatInfoService.findAppId(appid));
		}
		Page<WechatFans> page = dao.selectFans(appids, nickname, tagid, black, pageable);
		for (WechatFans wechatFans : page.getContent()) {
			String name = infos.stream()
					.filter(x -> wechatFans.getAppId().equals(x.getAppId())).findFirst().get().getWechatName();
			wechatFans.setWechatName(name);
		}
