...
public interface ChannelDao extends IBaseDao<Channel, Integer>, ChannelDaoExt {
    ...
    @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	List<Channel> findBySiteIdAndHasDeletedOrderBySortNumAscCreateTimeDesc(Integer siteId,Boolean hasDeleted);

	@QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	List<Channel> findByWorkflowIdInAndHasDeleted(Integer[] workflowIds,Boolean hasDeleted);

	@Modifying
	@Query(value = "update  Channel channel set channel.hasDeleted=true where channel.siteId=?1")
	void deleteBySiteId(Integer siteId);

    /**
     * 通过id查询未加入回收站，未被逻辑删除的栏目
     * @param id            栏目id
     ...
     */
    @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	Channel findByIdAndRecycleAndHasDeleted(Integer id,Boolean recycle,Boolean hasDeleted);
}
