...
public class Channel extends com.jeecms.common.base.domain.AbstractTreeDomain<Channel, Integer>
        ...
        public List<Channel> getChildAll() {
                return getListForSelect(null, true,false);
        }
         ...
         */
        @Transient
        public List<Channel> getListForSelect(Set<Channel> rights, boolean containNotDisplay,Boolean recycle) {
                return getListForSelect(rights, null, containNotDisplay, recycle);
        }
         ...
         */
        @Transient
        public List<Channel> getListForSelect(Set<Channel> rights, Channel exclude, boolean containNotDisplay, Boolean recycle) {
                List<Channel> list = new ArrayList<Channel>((getRgt() - getLft()) / 2);
                addChildToList(list, this, rights, exclude, containNotDisplay, recycle);
                return list;
        }
        ...
        public static List<Channel> getListForSelect(List<Channel> topList, Set<Channel> rights,
                        boolean containNotDisplay,Boolean recycle) {
                return getListForSelect(topList, rights, null, containNotDisplay,recycle);
        }
        ...
        public static List<Channel> getListForSelect(List<Channel> topList, Set<Channel> rights, Channel exclude,
                ...
                for (Channel c : topList) {
                        addChildToList(list, c, rights, exclude, containNotDisplay, recycle);
                }
                return list;
        }
 ...
 *                有权限的栏目，为null不控制权限。
        ...
        private static void addChildToList(List<Channel> list, Channel channel, Set<Channel> rights, Channel exclude,
                ...
                addChildToList(list, channel, containNotDisplay, recycle);
        }

        private static void addChildToList(List<Channel> list, Channel channel, boolean containNotDisplay, Boolean recycle) {
                if (!containNotDisplay) {
                        if (!channel.getDisplay()) {
                                return;
                        }
                }
                if(channel.getHasDeleted()!=null&&channel.getHasDeleted()){
                        return;
                }
                if(recycle!=null&&!recycle.equals(channel.getRecycle())){
                                return;
                }
                ...
                for (Channel c : child) {
                        addChildToList(list, c, containNotDisplay, recycle);
                }
        }
...
//                }
        ...
        public boolean getBrowsingContentAble() {
                ...
                return user.getBrowsingContentChannelIds().contains(getId());
        }
        ...
        public Map<String, ChannelAttr> getAttr() {
            ...
            Map<String, ChannelAttr> map = new HashMap<>();
                    ...
                    if (CmsModelConstant.MANY_CHOOSE.equals(attr.getAttrType())
                        ...
                        if (item != null) {
                            ...
                            if (attr.getAttrValue() != null) {
                                JSONObject attrValueJson = parseObject(attr.getAttrValue());
                                String obj = toJSONString(attrValueJson.get(s));
                                if (obj.startsWith("[")) {
                                    value = obj.substring(1, obj.length() - 1).split(",");
                                } else {
                                    value = obj.split(",");
                                }
                                other = toJSONString(attrValueJson.get("attrValue"));
                            }
                            if (value != null) {
                                for (Object o : array) {
                        ...
                        System.out.println("This is attrValue: " + labelValue.toString());
                        ...
                        if (item != null) {
                            ...
                            if (attr.getAttrValue() != null) {
                                JSONObject attrValueJson = parseObject(attr.getAttrValue());
                                value = toJSONString(attrValueJson.get(s));
                                other = toJSONString(attrValueJson.get("attrValue"));
                            }
                            if (value != null) {
                                for (Object o : array) {
                                    JSONObject json = (JSONObject) o;
                                    Integer key = json.getInteger(s);
                                    String label = json.getString("label");
        ...
        public Map<String, String> getTxtMap() {
                List<ChannelTxt> attrs = getTxts();
                Map<String, String> map = new HashMap<>();
                if (attrs != null && !attrs.isEmpty()) {
                        for(ChannelTxt attr:attrs){
                                map.put(attr.getAttrKey(),attr.getAttrTxt());
                        }
                }
                return map;
        }
