...
public class ContentPublishRecordServiceImpl extends BaseServiceImpl<ContentPublishRecord, ContentPublishRecordDao, Integer>
    ...
    public FlowContributionVo views(Integer siteId) throws GlobalException {
        ...
        if (!contentFlowVos.isEmpty()) {
            ...
            //首页
            long index = contentFlowVos.stream().filter(x -> x.getPageType().equals(PAGE_TYPE_INDEX)).count();
            vo.setIndex((int) index);
            //栏目页
            List<ContentFlowVo> channel = contentFlowVos.stream().filter(x -> x.getPageType().equals(PAGE_TYPE_CHANNEL))
            ...
            //内容页
            List<ContentFlowVo> content = contentFlowVos.stream().filter(x -> x.getPageType().equals(PAGE_TYPE_CONTENT))
                    .collect(Collectors.toList());
            if (!content.isEmpty()) {
                List<PublishVo> list = new ArrayList<>(16);
                //聚合栏目
